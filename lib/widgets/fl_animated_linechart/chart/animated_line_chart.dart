...
import 'package:salamatiman/widgets/fl_animated_linechart/chart/area_line_chart.dart';
...
import 'package:salamatiman/widgets/fl_animated_linechart/common/animated_path_util.dart';
...
class ChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawGridOnlyBorderBottom(canvas, size);
    _drawGridOnlyBorderLeft(canvas, size);
  ...
  void _drawAxisValues(Canvas canvas, Size size) {
    ...
    for (int c = 0; c <= (_stepCount + 1); c++) {
      _drawRotatedText(
          ...
          size.height - (LineChart.axisOffsetPX - 0),
          pi * 1.7);
    }
  }
  ...
  void _drawLines(Size size, Canvas canvas) {
    ...
    _chart.lines.forEach((chartLine) {
      ...
      if (_progress < 1.0) {
        ...
        //       Offset(p.chartPoint.x, p.chartPoint.y), 5, _linePainter));
        // }
        if (drawCircles) {
          //نقطه
          points.forEach((p) => canvas.drawCircle(
              Offset(p.chartPoint.x, p.chartPoint.y),
              6.5,
              Paint()
                ..color = Colors.red
                ..style = PaintingStyle.fill
                ..strokeCap = StrokeCap.round));
          points.forEach((p) => canvas.drawPath(
              Path()
                ..addRect(Rect.fromPoints(
                    Offset(p.chartPoint.x, p.chartPoint.y),
                    Offset(p.chartPoint.x, p.chartPoint.y)))
                ..addOval(Rect.fromPoints(
                    Offset(p.chartPoint.x + 7, p.chartPoint.y + 7),
                    Offset(p.chartPoint.x - 7, p.chartPoint.y - 7)))
  ...
  void _drawGridOnlyBorderBottom(Canvas canvas, Size size) {
    canvas.drawRect(
        Rect.fromLTWH(
  ...
  void _drawGridOnlyBorderLeft(Canvas canvas, Size size) {
    canvas.drawRect(
        Rect.fromLTWH(
            _chart.xAxisOffsetPX, 0, 1, size.height - LineChart.axisOffsetPX),
        Paint()..color = Colors.black26);

    canvas.drawRect(
        Rect.fromLTWH(size.width, 0, 1, size.height - LineChart.axisOffsetPX),
        Paint()..color = Colors.black26);
  }
